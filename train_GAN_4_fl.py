### GEBRUIKTE MODEL VOOR HET UITGEBREIDE GAN ###
...
def one_hot_to_index(array):
    ...
    array = torch.matmul(array.float(), index_values)
...
if __name__ == "__main__":
    ...
    optimizerd1 = optim.Adam(dis.parameters(), lr=0.0002, betas=(0.05, 0.999))      # learning rate normaal 0.0002, lower learning rate = 0.00004
    criteriond2 = nn.BCELoss()
    #optimizerd2 = optim.SGD(gen.parameters(), lr=0.001, momentum=0.9)
    optimizerd2 = optim.Adam(gen.parameters(), lr=0.0002, betas=(0.05, 0.999))      # learning rate normaal 0.0002, lower learning rate = 0.00004
    fuzzy_loss = fl.FuzzyLogic(1)   # 0 is hamacher, standaard
    ...
    torch.save(gen.state_dict(), 't1_weights_batches_' + str(0) + '.pth')
    print("product norm")
    # start training loop
    for epoch in range(epochs):
        for i, inp_real_x in enumerate(loader, 1):
            ...
            dis_inp_fake_x = gen(inp_fake_x_gen).detach()  # detach zodat je niet de afgeleiden naar gewichten van de generator berekent?
            ...
            if i % d_steps == 0:
                for g_step in range(g_steps):
                    ...
                    if epoch < 6:
                        ...
                        gen_loss = criteriond2(dis_out_gen_training, Variable(torch.ones(batch_size, 1))) + (epoch - 5) * 0.0075 * fuzzy_loss(gen_out)
                    else:
                        gen_loss = criteriond2(dis_out_gen_training, Variable(torch.ones(batch_size, 1))) + 0.03 * fuzzy_loss(gen_out)
                    #gen_loss = criteriond2(dis_out_gen_training, Variable(torch.ones(batch_size, 1))) + 0.125 * fuzzy_loss(gen_out)
                    gen_loss.backward()
                    optimizerd2.step()
            ...
            if (i % 50 == 0):
                b = epoch * 250 + i
                torch.save(gen.state_dict(), 't1_weights_batches_' + str(b) + '.pth')
